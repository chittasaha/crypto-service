...
use coingecko_rs::{CoinGeckoClient, params::{MarketsOrder, PriceChangePercentage}};
use crypto_service::models::{Currency, Token, GetTokensRequest};
...
#[post("/tokens")]
async fn get_tokens(req: web::Json<GetTokensRequest>) -> Result<String, Box<dyn std::error::Error>> {
    
    let client = CoinGeckoClient::default();

    let base_currency = &req.base_currency;
    ...
    let ids = &req.ids;
    let coins = client.coins_markets(&base_currency, &ids, None,
    ...
    for coin in coins
    {
        ...
        let ath_change_percent= coin.ath_change_percentage.unwrap();
        let atl_change_percent= coin.atl_change_percentage.unwrap();     
        let change_last_24_hours = coin.price_change_percentage24_h.unwrap();
